...
from bigframes_vendored.sqlglot.tokens import Token, TokenError
...
class Expression(metaclass=_Expression):
                        ...
                        if t is list:
                            for x in v:
                                if x is not None and x is not False:
                                    hash_ = hash(
                                        (hash_, k, x.lower() if type(x) is str else x)
                                    )
                                else:
                                    hash_ = hash((hash_, k))
    ...
    def __reduce__(self) -> t.Tuple[t.Callable, t.Tuple[t.List[t.Dict[str, t.Any]]]]:
        from bigframes_vendored.sqlglot.serde import dump, load
    ...
    def is_leaf(self) -> bool:
        return not any(
            isinstance(v, (Expression, list)) and v for v in self.args.values()
        )
    ...
    def dump(self):
        ...
        """
        from bigframes_vendored.sqlglot.serde import dump
    ...
    def load(cls, obj):
        ...
        """
        from bigframes_vendored.sqlglot.serde import load
...
class TimeUnit(Expression):
    ...
    def __init__(self, **args):
        unit = args.get("unit")
        if type(unit) in self.VAR_LIKE and not (
...
def maybe_parse(
    ...
    **opts,
    ...
    return bigframes_vendored.sqlglot.parse_one(sql, read=dialect, into=into, **opts)
    ...
    **opts,
    ...
    if isinstance(using_clause := merge.args.get("using"), Alias):
        using_clause.replace(
            alias_(using_clause.this, using_clause.args["alias"], table=True)
        )
