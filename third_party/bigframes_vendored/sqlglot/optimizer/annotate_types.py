...
if t.TYPE_CHECKING:
    ...
    from bigframes_vendored.sqlglot.typing import ExpressionMetadataType
...
class TypeAnnotator(metaclass=_TypeAnnotator):
    ...
    def _set_type(
        ...
        if (
            ...
            prev_type and t.cast(exp.DataType, prev_type).this == exp.DataType.Type.NULL
        ):
    ...
    def annotate_scope(self, scope: Scope) -> None:
        ...
        if self.dialect.QUERY_RESULTS_ARE_STRUCTS and isinstance(
    ...
    def _annotate_binary(self, expression: B) -> B:
        ...
        if not left or not right:
            ...
            logger.warning(
                f"Failed to annotate badly formed binary expression: {expression_sql}"
            )
        ...
        *args: str | exp.Expression,
            ...
            for expr in ensure_list(expressions):
                ...
                if not expr_type.is_type(exp.DataType.Type.UNKNOWN):
                    if isinstance(expr, exp.Literal):
                        literal_type = self._maybe_coerce(
                            literal_type or expr_type, expr_type
                        )
                    else:
        ...
        if nested_type:
            ...
            if self.dialect.PRIORITIZE_NON_LITERAL_TYPES:
                literal_this_type = (
                    literal_type.this
                    if isinstance(literal_type, exp.DataType)
                    else literal_type
                )
        ...
        self._set_type(
            expression,
            result_type or self._maybe_coerce(non_literal_type, literal_type),
        )
    ...
    def _annotate_struct(self, expression: exp.Struct) -> exp.Struct:
        ...
        self._set_type(
            expression,
            exp.DataType(
                this=exp.DataType.Type.STRUCT, expressions=expressions, nested=True
            ),
        )
        return expression
