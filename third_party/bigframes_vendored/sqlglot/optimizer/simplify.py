...
from bigframes_vendored.sqlglot.helper import first, merge_ranges, while_changing
from bigframes_vendored.sqlglot.optimizer.annotate_types import TypeAnnotator
from bigframes_vendored.sqlglot.optimizer.scope import find_all_in_scope, walk_in_scope
...
def annotate_types_on_change(func):
    @wraps(func)
    def _func(
        self, expression: exp.Expression, *args, **kwargs
    ) -> t.Optional[exp.Expression]:
...
def propagate_constants(expression, root=True):
    ...
    if (
        ...
        and bigframes_vendored.sqlglot.optimizer.normalize.normalized(
            expression, dnf=True
        )
    ):
        constant_mapping = {}
...
class Simplifier:
    def __init__(
        self, dialect: DialectType = None, annotate_new_expressions: bool = True
    ):
        ...
        for node in expression.walk(
            ...
            if isinstance(node, exp.Condition):
                simplified = while_changing(
                    node,
                    lambda e: self._simplify(
                        e, constant_propagation, coalesce_simplification
                    ),
                )
    ...
    def rewrite_between(self, expression: exp.Expression) -> exp.Expression:
        ...
        if isinstance(expression, exp.Between):
            ...
            expression = exp.and_(
                ...
                exp.LTE(
                    this=expression.this.copy(), expression=expression.args["high"]
                ),
                copy=False,
            )
    ...
    def simplify_not(self, expression: exp.Expression) -> exp.Expression:
        ...
        if isinstance(expression, exp.Not):
            ...
            if this.__class__ in self.COMPLEMENT_COMPARISONS:
                ...
                return self.COMPLEMENT_COMPARISONS[this.__class__](
            ...
            if is_false(this):
                return exp.true()
            if (
                isinstance(this, exp.Not)
                and self.dialect.SAFE_TO_ELIMINATE_DOUBLE_NEGATION
            ):
                inner = this.this
    ...
    def _simplify_comparison(self, expression, left, right, or_=False):
        if isinstance(left, self.COMPARISONS) and isinstance(right, self.COMPARISONS):
            ...
            columns = {
                m
                for m in matching
                if not _is_constant(m) and not m.find(*self.NONDETERMINISTIC)
            }
    ...
    def uniq_sort(self, expression, root=True):
        ...
        if isinstance(expression, exp.Connector) and (
            root or not expression.same_parent
        ):
    ...
    def absorb_and_eliminate(self, expression, root=True):
        ...
        if isinstance(expression, self.AND_OR) and (root or not expression.same_parent):
            ...
            for op in ops:
                ...
                if any(
                    any(subset < superset for subset in subops[i]) for i in superset
                ):
    ...
    def simplify_literals(self, expression, root=True):
        if isinstance(expression, exp.Binary) and not isinstance(
        ...
        if type(expression) in self.INVERSE_DATE_OPS:
            return (
                self._simplify_binary(
                    expression, expression.this, expression.interval()
                )
                or expression
            )
    ...
    def _simplify_binary(self, expression, a, b):
        ...
        if a.is_number and b.is_number:
            ...
            if isinstance(expression, exp.Sub):
                return (
                    exp.Literal.number(num_a - num_b) if a.parent is b.parent else None
                )
    ...
    def simplify_conditionals(self, expression):
        ...
        elif isinstance(expression, exp.If) and not isinstance(
            expression.parent, exp.Case
        ):
    ...
    def simplify_startswith(self, expression: exp.Expression) -> exp.Expression:
        ...
        Example:
            >>> from bigframes_vendored.sqlglot import parse_one
    ...
    def simplify_datetrunc(self, expression: exp.Expression) -> exp.Expression:
        ...
        if isinstance(expression, self.DATETRUNCS):
            ...
            if date and expression.unit:
                return date_literal(
                    date_floor(date, expression.unit.name.lower(), self.dialect),
                    trunc_type,
                )
    ...
    def sort_comparison(self, expression: exp.Expression) -> exp.Expression:
        if expression.__class__ in self.COMPLEMENT_COMPARISONS:
            ...
            if (
                ...
                return self.INVERSE_COMPARISONS.get(
                    expression.__class__, expression.__class__
                )(this=r, expression=l)
        return expression
