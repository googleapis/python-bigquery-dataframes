...
from bigframes_vendored.sqlglot.errors import UnsupportedError
from bigframes_vendored.sqlglot.helper import find_new_name, name_sequence, seq_get

if t.TYPE_CHECKING:
    from bigframes_vendored.sqlglot._typing import E
    from bigframes_vendored.sqlglot.generator import Generator
...
def eliminate_full_outer_join(expression: exp.Expression) -> exp.Expression:
    ...
    if isinstance(expression, exp.Select):
        ...
        if len(full_outer_joins) == 1:
            ...
            tables = (
                expression.args["from_"].alias_or_name,
...
def ensure_bools(expression: exp.Expression) -> exp.Expression:
    ...
    from bigframes_vendored.sqlglot.optimizer.canonicalize import ensure_bools
...
def eliminate_join_marks(expression: exp.Expression) -> exp.Expression:
    ...
    from bigframes_vendored.sqlglot.optimizer.normalize import normalize, normalized
...
def inherit_struct_field_names(expression: exp.Expression) -> exp.Expression:
    ...
    if (
        ...
        for struct in expression.expressions[1:]:
            if not isinstance(struct, exp.Struct) or len(struct.expressions) != len(
                field_names
            ):
                continue
