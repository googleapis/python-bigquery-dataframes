# Contains code from https://github.com/ibis-project/ibis/blob/9.2.0/ibis/expr/types/generic.py
...
class Value(Expr):
    ...
    def isin(self, values: Value | Sequence[Value]) -> ir.BooleanValue:
        ...
        └───────────────────┘
        """
        from bigframes_vendored.ibis.expr.types import ArrayValue
...
class Column(Value, _FixedTextJupyterMixin):
    ...
    def topk(self, k: int, by: ir.Value | None = None) -> ir.Table:
        ...
        Table
            A top-k expression
        """
        from bigframes_vendored.ibis.expr.types.relations import bind
