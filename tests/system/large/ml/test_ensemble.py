...
def test_xgbregressor_dart_booster_num_parallel_tree(
    ...
    assert 10000 <= int(result.mean_squared_error) <= 100000
    assert "mean_absolute_error" in result.columns
    assert "mean_squared_log_error" in result.columns
    assert "median_absolute_error" in result.columns
    assert "r2_score" in result.columns
    assert "explained_variance" in result.columns
