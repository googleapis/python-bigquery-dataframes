...
def test_model_eval(
    ...
    model = bigframes.ml.core.BqmlModel(session, penguins_model)
    result = model.evaluate().compute()
...
def test_model_eval_with_data(session, penguins_model, penguins_df_no_index):
    model = bigframes.ml.core.BqmlModel(session, penguins_model)
    result = model.evaluate(penguins_df_no_index.dropna()).compute()
    expected = pandas.DataFrame(
        {
...
def test_model_predict(session, penguins_model):
    model = bigframes.ml.core.BqmlModel(session, penguins_model)
