...
@pytest.mark.parametrize(
    ...
    [
        ...
        ([datetime.date(2023, 1, 1)], pa.date64(), "DATE"),
        (
...
def test_read_arrow_type_mappings(session, data, arrow_type, expected_bq_type_kind):
    ...
    col_id = df._block.label_to_col_id["col"]
    bigquery_schema = df._block.expr.schema.to_bigquery()
    field = [field for field in bigquery_schema if field.name == col_id][0]
...
def test_read_arrow_list_type(session):
    ...
    col_id = df._block.label_to_col_id["list_col"]
    bigquery_schema = df._block.expr.schema.to_bigquery()
    field = [field for field in bigquery_schema if field.name == col_id][0]
...
def test_read_arrow_struct_type(session):
    ...
    col_id = df._block.label_to_col_id["struct_col"]
    bigquery_schema = df._block.expr.schema.to_bigquery()
    field = [field for field in bigquery_schema if field.name == col_id][0]
