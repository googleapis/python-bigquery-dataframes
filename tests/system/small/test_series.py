...
def test_series_construct_nullable_ints():
    ...
    assert_series_equal(bf_result, expected)
...
def test_series_construct_pandas(scalars_dfs):
    ...
    assert_series_equal(bf_result.to_pandas(), pd_result)
...
def test_series_construct_w_dtype(dtype):
    ...
    assert_series_equal(series.to_pandas(), expected)
...
def test_series_construct_w_dtype_for_array_string():
    ...
    assert_series_equal(series.to_pandas(), expected, check_dtype=check_dtype)
...
def test_series_construct_w_nested_json_dtype():
    ...
    if hasattr(pa, "JsonType"):
        ...
        assert_series_equal(s.to_pandas(), s2.to_pandas())
...
def test_series___setitem__(scalars_dfs, index_col, key, value):
    ...
    assert_series_equal(bf_series.to_pandas(), pd_series)
...
def test_divmods_series(scalars_dfs, col_x, col_y, method):
    ...
    if bf_div_result.dtype == pd.Int64Dtype():
        assert_series_equal(pd_div_result, bf_div_result.to_pandas())
    else:
        assert_series_equal(pd_div_result, bf_div_result.astype("Float64").to_pandas())

    if bf_mod_result.dtype == pd.Int64Dtype():
        assert_series_equal(pd_mod_result, bf_mod_result.to_pandas())
    else:
        assert_series_equal(pd_mod_result, bf_mod_result.astype("Float64").to_pandas())
...
def test_series_add_different_table_default_index(
    ...
    assert_series_equal(bf_result.sort_index(), pd_result.sort_index())
...
def test_copy(scalars_df_index, scalars_pandas_df_index):
    ...
    assert_series_equal(bf_copy.to_pandas(), pd_copy)
...
def test_loc_setitem_cell(scalars_df_index, scalars_pandas_df_index):
    ...
    assert_series_equal(bf_original.to_pandas(), pd_original)
...
def test_duplicated(scalars_df_index, scalars_pandas_df_index, keep, col_name):
    ...
    assert_series_equal(pd_series, bf_series, check_dtype=False)
...
def test_series_values(scalars_df_index, scalars_pandas_df_index):
    ...
    assert_series_equal(pd.Series(bf_result), pd.Series(pd_result), check_dtype=False)
...
def test_series_astype_python(session):
    ...
    assert_series_equal(result, exepcted)
...
def test_float_astype_json(errors):
    ...
    assert_series_equal(bf_result.to_pandas(), expected_result)
...
def test_string_astype_json_in_safe_mode():
    ...
    expected.index = expected.index.astype("Int64")
    assert_series_equal(bf_result.to_pandas(), expected)
