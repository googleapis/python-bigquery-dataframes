...
class Length1Transformer(CustomTransformer):
    ...
    LEN1_BQSQL_RX = re.compile("^CASE WHEN (?P<colname>[a-z][a-z0-9_]*) IS NULL THEN (?P<defaultvalue>[-]?[0-9]+) ELSE LENGTH[(](?P=colname)[)] END$", flags=re.IGNORECASE)
...
class Length2Transformer(CustomTransformer):
    _CTID = "LEN2"
    _DEFAULT_VALUE_DEFAULT = -1
    LEN2_BQSQL_RX = re.compile("^CASE WHEN (?P<colname>[a-z][a-z0-9_]*) .*$", flags=re.IGNORECASE)
