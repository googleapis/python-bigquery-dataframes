  ...
  FROM (
    SELECT
      IF(
        ...
        ARRAY(
          SELECT
            ibis_bq_arr_range_xtpfaha7xvgb3gbskkmi7kfqcq
          FROM UNNEST(generate_array(0, GREATEST(1, LEAST(ARRAY_LENGTH(`t0`.`int_list_col`))), 1)) AS ibis_bq_arr_range_xtpfaha7xvgb3gbskkmi7kfqcq
          WHERE
            ibis_bq_arr_range_xtpfaha7xvgb3gbskkmi7kfqcq <> GREATEST(1, LEAST(ARRAY_LENGTH(`t0`.`int_list_col`)))
        ),
        []
