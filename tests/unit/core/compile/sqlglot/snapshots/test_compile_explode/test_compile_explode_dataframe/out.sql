  ...
  FROM (
    SELECT
      IF(
        ...
        ARRAY(
          SELECT
            ibis_bq_arr_range_pr3gi7iu6fgmbf65drdhebyp2u
          FROM UNNEST(generate_array(
            0,
            GREATEST(1, LEAST(ARRAY_LENGTH(`t0`.`int_list_col`), ARRAY_LENGTH(`t0`.`string_list_col`))),
            1
          )) AS ibis_bq_arr_range_pr3gi7iu6fgmbf65drdhebyp2u
          WHERE
            ibis_bq_arr_range_pr3gi7iu6fgmbf65drdhebyp2u <> GREATEST(1, LEAST(ARRAY_LENGTH(`t0`.`int_list_col`), ARRAY_LENGTH(`t0`.`string_list_col`)))
        ),
        []
