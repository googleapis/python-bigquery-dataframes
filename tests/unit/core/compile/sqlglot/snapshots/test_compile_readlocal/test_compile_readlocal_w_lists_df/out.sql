...
(SELECT
  ...
  `t0`.`bfuid_col_812` AS `bfuid_col_813`
FROM (
  SELECT
    *
  FROM UNNEST(ARRAY<STRUCT<`level_0` INT64, `column_0` ARRAY<INT64>, `column_1` ARRAY<BOOLEAN>, `column_2` ARRAY<FLOAT64>, `column_3` ARRAY<STRING>, `column_4` ARRAY<STRING>, `column_5` ARRAY<FLOAT64>, `column_6` ARRAY<STRING>, `bfuid_col_812` INT64>>[STRUCT(
    0,
    [1],
    [TRUE],
    [1.2, 2.3],
...
ORDER BY `bfuid_col_813` ASC NULLS LAST