...
class SQLCompiler(bigframes.session.executor.Executor):
...
class SQLCompilerSession(bigframes.session.Session):
    def __init__(self):
        ...
        self._session_id: str = "sqlglot_test_session"
        self._objects: list[
        ...
        self._executor = SQLCompiler()
        self._loader = None  # type: ignore

        self._clients_provider = bigframes.session.clients.ClientsProvider(
            project="compile.sqlglot",
            location="unit",
        )
        self._clients_provider._bqclient = mock.create_autospec(spec=bigquery.Client)
