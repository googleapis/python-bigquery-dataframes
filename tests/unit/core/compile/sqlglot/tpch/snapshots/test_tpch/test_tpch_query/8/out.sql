WITH `bfcte_0` AS (
  SELECT
    ...
    `N_NAME` AS `bfcol_1`
  FROM `bigframes-dev-perf`.`tpch_0001t`.`NATION` AS `bft_6` FOR SYSTEM_TIME AS OF '2026-03-30T18:00:00'
), `bfcte_1` AS (
  SELECT
  ...
  SELECT
    ...
    `R_NAME` AS `bfcol_3`
  FROM `bigframes-dev-perf`.`tpch_0001t`.`REGION` AS `bft_5` FOR SYSTEM_TIME AS OF '2026-03-30T18:00:00'
), `bfcte_3` AS (
  SELECT
    `C_CUSTKEY` AS `bfcol_6`,
    `C_NATIONKEY` AS `bfcol_7`
  FROM `bigframes-dev-perf`.`tpch_0001t`.`CUSTOMER` AS `bft_4` FOR SYSTEM_TIME AS OF '2026-03-30T18:00:00'
), `bfcte_4` AS (
  SELECT
    `O_ORDERKEY` AS `bfcol_8`,
    `O_CUSTKEY` AS `bfcol_9`,
    `O_ORDERDATE` AS `bfcol_10`
  FROM `bigframes-dev-perf`.`tpch_0001t`.`ORDERS` AS `bft_3` FOR SYSTEM_TIME AS OF '2026-03-30T18:00:00'
), `bfcte_5` AS (
  SELECT
    `S_SUPPKEY` AS `bfcol_11`,
    `S_NATIONKEY` AS `bfcol_12`
  FROM `bigframes-dev-perf`.`tpch_0001t`.`SUPPLIER` AS `bft_2` FOR SYSTEM_TIME AS OF '2026-03-30T18:00:00'
), `bfcte_6` AS (
  SELECT
  ...
  FROM `bigframes-dev-perf`.`tpch_0001t`.`LINEITEM` AS `bft_1` FOR SYSTEM_TIME AS OF '2026-03-30T18:00:00'
), `bfcte_7` AS (
  SELECT
    `P_PARTKEY` AS `bfcol_18`,
    `P_TYPE` AS `bfcol_19`
  FROM `bigframes-dev-perf`.`tpch_0001t`.`PART` AS `bft_0` FOR SYSTEM_TIME AS OF '2026-03-30T18:00:00'
), `bfcte_8` AS (
  SELECT
