WITH `bfcte_0` AS (
  SELECT
    ...
    `R_NAME` AS `bfcol_1`
  FROM `bigframes-dev-perf`.`tpch_0001t`.`REGION` AS `bft_4` FOR SYSTEM_TIME AS OF '2026-03-30T18:00:00'
), `bfcte_1` AS (
  SELECT
  ...
  FROM `bigframes-dev-perf`.`tpch_0001t`.`PARTSUPP` AS `bft_1` FOR SYSTEM_TIME AS OF '2026-03-30T18:00:00'
), `bfcte_6` AS (
  SELECT
