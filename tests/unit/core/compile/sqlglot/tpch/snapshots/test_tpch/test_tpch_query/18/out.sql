WITH `bfcte_0` AS (
  SELECT
    ...
    `C_NAME` AS `bfcol_1`
  FROM `bigframes-dev-perf`.`tpch_0001t`.`CUSTOMER` AS `bft_2` FOR SYSTEM_TIME AS OF '2026-03-30T18:00:00'
), `bfcte_1` AS (
  SELECT
  ...
  FROM `bigframes-dev-perf`.`tpch_0001t`.`ORDERS` AS `bft_0` FOR SYSTEM_TIME AS OF '2026-03-30T18:00:00'
), `bfcte_3` AS (
  SELECT
    `bfcol_2`,
