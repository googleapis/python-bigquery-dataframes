WITH `bfcte_0` AS (
  SELECT
    ...
    `N_NAME` AS `bfcol_1`
  FROM `bigframes-dev-perf`.`tpch_0001t`.`NATION` AS `bft_5` FOR SYSTEM_TIME AS OF '2026-03-30T18:00:00'
), `bfcte_1` AS (
  SELECT
    `O_ORDERKEY` AS `bfcol_2`,
    `O_ORDERDATE` AS `bfcol_3`
  FROM `bigframes-dev-perf`.`tpch_0001t`.`ORDERS` AS `bft_4` FOR SYSTEM_TIME AS OF '2026-03-30T18:00:00'
), `bfcte_2` AS (
  SELECT
  ...
  FROM `bigframes-dev-perf`.`tpch_0001t`.`PARTSUPP` AS `bft_2` FOR SYSTEM_TIME AS OF '2026-03-30T18:00:00'
), `bfcte_4` AS (
  SELECT
