WITH `bfcte_0` AS (
  ...
  SELECT
    ...
    `N_NAME` AS `bfcol_3`
  FROM `bigframes-dev-perf`.`tpch_0001t`.`NATION` AS `bft_2` FOR SYSTEM_TIME AS OF '2026-03-30T18:00:00'
), `bfcte_2` AS (
  SELECT
    `S_SUPPKEY` AS `bfcol_4`,
    `S_NAME` AS `bfcol_5`,
    `S_NATIONKEY` AS `bfcol_6`
  FROM `bigframes-dev-perf`.`tpch_0001t`.`SUPPLIER` AS `bft_1` FOR SYSTEM_TIME AS OF '2026-03-30T18:00:00'
), `bfcte_3` AS (
  SELECT
