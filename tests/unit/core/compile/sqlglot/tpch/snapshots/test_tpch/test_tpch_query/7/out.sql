WITH `bfcte_0` AS (
  ...
  FROM `bigframes-dev-perf`.`tpch_0001t`.`NATION` AS `bft_4` FOR SYSTEM_TIME AS OF '2026-03-30T18:00:00'
  WHERE
  ...
  FROM `bigframes-dev-perf`.`tpch_0001t`.`SUPPLIER` AS `bft_3` FOR SYSTEM_TIME AS OF '2026-03-30T18:00:00'
), `bfcte_2` AS (
  SELECT
  ...
  FROM `bigframes-dev-perf`.`tpch_0001t`.`LINEITEM` AS `bft_2` FOR SYSTEM_TIME AS OF '2026-03-30T18:00:00'
  WHERE
    (
  ...
  FROM `bigframes-dev-perf`.`tpch_0001t`.`ORDERS` AS `bft_1` FOR SYSTEM_TIME AS OF '2026-03-30T18:00:00'
), `bfcte_4` AS (
  SELECT
    `C_CUSTKEY` AS `bfcol_11`,
    `C_NATIONKEY` AS `bfcol_12`
  FROM `bigframes-dev-perf`.`tpch_0001t`.`CUSTOMER` AS `bft_0` FOR SYSTEM_TIME AS OF '2026-03-30T18:00:00'
), `bfcte_5` AS (
  SELECT
  ...
  FROM `bfcte_4`
  INNER JOIN `bfcte_6`
    ON COALESCE(`bfcol_12`, 0) = COALESCE(`bfcol_38`, 0)
    AND COALESCE(`bfcol_12`, 1) = COALESCE(`bfcol_38`, 1)
), `bfcte_8` AS (
  SELECT
  ...
  FROM `bfcte_7`
  INNER JOIN `bfcte_3`
    ON COALESCE(`bfcol_40`, 0) = COALESCE(`bfcol_10`, 0)
    AND COALESCE(`bfcol_40`, 1) = COALESCE(`bfcol_10`, 1)
), `bfcte_9` AS (
  SELECT
  ...
  FROM `bfcte_8`
  INNER JOIN `bfcte_2`
    ON COALESCE(`bfcol_43`, 0) = COALESCE(`bfcol_31`, 0)
    AND COALESCE(`bfcol_43`, 1) = COALESCE(`bfcol_31`, 1)
), `bfcte_10` AS (
  SELECT
  ...
  FROM `bfcte_9`
  INNER JOIN `bfcte_1`
    ON COALESCE(`bfcol_45`, 0) = COALESCE(`bfcol_2`, 0)
    AND COALESCE(`bfcol_45`, 1) = COALESCE(`bfcol_2`, 1)
), `bfcte_11` AS (
  SELECT
    `bfcol_49`,
    `bfcol_50`,
  ...
  FROM `bfcte_10`
  INNER JOIN `bfcte_5`
    ON COALESCE(`bfcol_53`, 0) = COALESCE(`bfcol_36`, 0)
    AND COALESCE(`bfcol_53`, 1) = COALESCE(`bfcol_36`, 1)
  WHERE
    `bfcol_49` <> `bfcol_37`
), `bfcte_12` AS (
  SELECT
  ...
  FROM `bfcte_11`
  WHERE
    NOT `bfcol_85` IS NULL AND NOT `bfcol_84` IS NULL AND NOT `bfcol_87` IS NULL
  GROUP BY
    `bfcol_85`,
    `bfcol_84`,
    `bfcol_87`
