WITH `bfcte_0` AS (
  ...
  FROM `bigframes-dev-perf`.`tpch_0001t`.`SUPPLIER` AS `bft_5` FOR SYSTEM_TIME AS OF '2026-03-30T18:00:00'
), `bfcte_1` AS (
  SELECT
  ...
  FROM `bigframes-dev-perf`.`tpch_0001t`.`NATION` AS `bft_3` FOR SYSTEM_TIME AS OF '2026-03-30T18:00:00'
), `bfcte_3` AS (
  SELECT
    `R_REGIONKEY` AS `bfcol_32`
  FROM `bigframes-dev-perf`.`tpch_0001t`.`REGION` AS `bft_2` FOR SYSTEM_TIME AS OF '2026-03-30T18:00:00'
  WHERE
    `R_NAME` = 'ASIA'
), `bfcte_4` AS (
  SELECT
  ...
  FROM `bigframes-dev-perf`.`tpch_0001t`.`LINEITEM` AS `bft_0` FOR SYSTEM_TIME AS OF '2026-03-30T18:00:00'
), `bfcte_6` AS (
  SELECT
    `bfcol_4` AS `bfcol_35`,
    `bfcol_5` AS `bfcol_36`
