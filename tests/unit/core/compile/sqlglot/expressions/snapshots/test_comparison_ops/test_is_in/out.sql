SELECT
  COALESCE(`t0`.`bool_col` IN (TRUE, FALSE), FALSE) AS `bools`,
  COALESCE(`t0`.`int64_col` IN (1, 2, 3), FALSE) AS `ints`,
  (
    `t0`.`int64_col` IS NULL
  ) OR FALSE AS `ints_w_null`,
  ...
  COALESCE(FALSE, FALSE) AS `empty_wo_match_nulls`,
  ...
  (
    `t0`.`int64_col` IS NULL
  ) OR `t0`.`int64_col` IN (2) AS `mixed_with_null`,
  COALESCE(FALSE, FALSE) AS `bool_in_mixed`,
  (
    `t0`.`int64_col` IS NULL
  ) OR FALSE AS `only_null_match`
