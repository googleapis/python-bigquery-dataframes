...
class Session(
    ...
    def read_csv(
        ...
        **kwargs,
        ...
        if engine is None or engine != "bigquery":
    ...
    def _read_csv_w_bigquery_engine(
        ...
        *,
        ...
        elif bigframes.core.utils.is_list_like(header, allow_sets=False):
            raise NotImplementedError("Cannot support to specify a multi-index header.")
        elif header > 0:
            job_config.skip_leading_rows = header + 1

        table = self._temp_storage_manager.allocate_temp_table()
        df = self._loader._read_bigquery_load_job(
            filepath_or_buffer,
            table,
