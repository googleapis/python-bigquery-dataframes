...
class BigFrameNode:
    ...
    # Note, this is temporary, later on column ids will be offset-based
    @property
    @abc.abstractmethod
    def columns(self) -> Tuple[ID_TYPE, ...]:
        ...

    @property
    @abc.abstractmethod
    def dtypes(self) -> Tuple[bigframes.dtypes.Dtype, ...]:
        ...
...
class JoinNode(BigFrameNode):
    ...
    conditions: typing.Tuple[typing.Tuple[ID_TYPE, ID_TYPE], ...]
    ...
    def columns(self) -> Tuple[ID_TYPE, ...]:
        return tuple(
            range(len(self.left_child.columns) + len(self.right_child.columns))
        )

    @functools.cached_property
    def dtypes(self) -> Tuple[bigframes.dtypes.Dtype, ...]:
        return tuple(*self.left_child.dtypes, *self.right_child.dtypes)
...
class ConcatNode(BigFrameNode):
    ...
    @functools.cached_property
    def columns(self) -> Tuple[ID_TYPE, ...]:
        return tuple(range(len(self.children[0])))

    @functools.cached_property
    def dtypes(self) -> Tuple[bigframes.dtypes.Dtype, ...]:
        return self.children[0].dtypes

    @functools.cached_property
    def variables_introduced(self) -> int:
        """Defines the number of variables generated by the current node. Used to estimate query planning complexity."""
        return len(self.columns) + OVERHEAD_VARIABLES
...
class GbqTable:
    ...
    n_rows: int = field()
    cluster_cols: typing.Optional[Tuple[ID_TYPE, ...]]
...
class ReadTableNode(LeafNode):
    ...
    total_order_cols: Tuple[ID_TYPE, ...] = field()
...
class PromoteOffsetsNode(UnaryNode):
    ...
    @property
    def columns(self) -> Tuple[ID_TYPE, ...]:
        return (*self.child.columns, self.col_id)

    @property
    def dtypes(self) -> Tuple[bigframes.dtypes.Dtype, ...]:
        return (*self.child.dtypes, bigframes.dtypes.INT_DTYPE)
...
class ProjectionNode(UnaryNode):
    assignments: typing.Tuple[typing.Tuple[ex.Expression, ID_TYPE], ...]

    def __post_init__(self):
        input_types = {
            id: dtype for id, dtype in zip(self.child.columns, self.child.dtypes)
        }
    ...
    def dtypes(self) -> Tuple[bigframes.dtypes.Dtype, ...]:
        ...
        return tuple(
            bigframes.dtypes.dtype_for_etype(
                ex.output_type(input_types) for ex, _ in self.assignments
            )
        )
...
class RowCountNode(UnaryNode):
    ...
    @property
    def dtypes(self) -> Tuple[bigframes.dtypes.Dtype, ...]:
        return (bigframes.dtypes.INT_DTYPE,)
...
class AggregateNode(UnaryNode):
    aggregations: typing.Tuple[typing.Tuple[ex.Aggregation, ID_TYPE], ...]
    by_column_ids: typing.Tuple[ID_TYPE, ...] = tuple([])
    dropna: bool = True
    ...
    @property
    def columns(self) -> Tuple[ID_TYPE, ...]:
        return (*self.by_column_ids, *(col for _, col in self.aggregations))

    @functools.cached_property
    def dtypes(self) -> Tuple[bigframes.dtypes.Dtype, ...]:
        input_dtypes = {
            name: dtype for name, dtype in zip(self.child.columns, self.child.dtypes)
        }
        by_items = tuple(
            schemata.SchemaItem(id, input_dtypes[id]) for id in self.by_column_ids
        )
        agg_items = tuple(
            schemata.SchemaItem(
                id, bigframes.dtypes.dtype_for_etype(agg.output_type(input_dtypes))
            )
...
class WindowOpNode(UnaryNode):
    ...
    output_name: typing.Optional[ID_TYPE] = None
    ...
    @property
    def columns(self) -> Tuple[ID_TYPE, ...]:
        # TODO: Remove mutation case
        if (self.output_name is None) or self.output_name in self.child.columns:
            return self.child.columns
        return (*self.child.columns, self.output_name)
    ...
    def dtypes(self) -> Tuple[bigframes.dtypes.Dtype, ...]:
        ...
        output_name = (
            self.output_name if (self.output_name is None) else self.column_name
        )
        # TODO: Remove mutation case
        if self.output_name in self.child.columns:
            return (
                new_item_dtype if name == output_name else dtype
                for name, dtype in zip(self.child.columns, self.child.dtypes)
            )
        return (*self.child.dtypes, new_item_dtype)
...
class ExplodeNode(UnaryNode):
    column_ids: typing.Tuple[ID_TYPE, ...]
