...
class InNode(BigFrameNode, AdditiveNode):
    ...
    @property
    def ids(self) -> Sequence[identifiers.ColumnId]:
        return sequences.ChainedSequence(self.left_child.ids, [self.indicator_col])
...
class JoinNode(BigFrameNode):
    ...
    @property
    def ids(self) -> Sequence[identifiers.ColumnId]:
        return sequences.ChainedSequence(self.left_child.ids, self.right_child.ids)
...
class ReadLocalNode(LeafNode):
    ...
    @property
    def ids(self) -> Sequence[identifiers.ColumnId]:
        ids = (item.id for item in self.scan_list.items)
        if self.offsets_col:
            return (*ids, self.offsets_col)
        else:
            return tuple(ids)
...
class PromoteOffsetsNode(UnaryNode, AdditiveNode):
    ...
    @property
    def ids(self) -> Sequence[identifiers.ColumnId]:
        return sequences.ChainedSequence(self.child.ids, [self.col_id])
...
class SelectionNode(UnaryNode):
    ...
    @property
    def ids(self) -> Sequence[identifiers.ColumnId]:
        return tuple(ref.id for ref in self.input_output_pairs)
...
class ProjectionNode(UnaryNode, AdditiveNode):
    ...
    def ids(self) -> Sequence[identifiers.ColumnId]:
        return sequences.ChainedSequence(
            self.child.ids, [i[1] for i in self.assignments]
        )
...
class AggregateNode(UnaryNode):
    ...
    def ids(self) -> Sequence[identifiers.ColumnId]:
        ...
        agg_items = (id for agg, id in self.aggregations)
        return tuple(itertools.chain(by_ids, agg_items))
...
class WindowOpNode(UnaryNode, AdditiveNode):
    ...
    @property
    def ids(self) -> Sequence[identifiers.ColumnId]:
        return sequences.ChainedSequence(self.child.ids, (self.output_name,))
...
class ResultNode(UnaryNode):
    ...
    @property
    def ids(self) -> Sequence[identifiers.ColumnId]:
        return tuple(identifiers.ColumnId(output) for _, output in self.output_cols)
