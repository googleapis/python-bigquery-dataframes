...
def _interpolate_column(
    ...
    window_ordering = (ordering.OrderingExpression(ex.deref_name(x_values)),)
...
def nsmallest(
    ...
    order_refs = [
        ordering.OrderingExpression(
            ex.deref_name(col_id), direction=ordering.OrderingDirection.ASC
        )
...
def nlargest(
    ...
    order_refs = [
        ordering.OrderingExpression(
            ex.deref_name(col_id), direction=ordering.OrderingDirection.DESC
        )
        for col_id in column_ids
    ]
    block = block.order_by(order_refs)
...
def _idx_extrema(
    ...
    for value_col in original_block.value_columns:
        ...
        order_refs = [
            ordering.OrderingExpression(ex.deref_name(value_col), direction),
            *[
