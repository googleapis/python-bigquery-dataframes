...
def try_reduce_to_table_scan(node: nodes.BigFrameNode) -> Optional[nodes.ReadTableNode]:
    # TODO: Shortcircuit for obvious non-candidates
...
def _(node: nodes.SelectionNode) -> nodes.BigFrameNode:
    if isinstance(node.child, nodes.ReadTableNode) and not node.double_references_ids:
        selection = {
            aliased_ref.ref.id: aliased_ref.id
