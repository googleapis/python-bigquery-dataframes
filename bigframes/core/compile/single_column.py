...
def join_by_column_ordered(
    ...
    l_value_mapping = {
        id: compile_ids.map_id_to_sql(i) for i, id in enumerate(left.column_ids)
    }
    r_value_mapping = {
        id: compile_ids.map_id_to_sql(i)
        for i, id in enumerate(right.column_ids, len(left.column_ids))
    }
    l_hidden_mapping = {
