...
def compile_join(node: nodes.JoinNode, ordered: bool = True):
    compiled_left = compile_node(node.left_child, ordered)
    compiled_right = compile_node(node.right_child, ordered)

    if ordered:
        compiled_left = compile_ordered(node.left_child)
        compiled_right = compile_ordered(node.right_child)
        ...
        compiled_left = compile_node(node.left_child, False)
        compiled_right = compile_node(node.right_child, False)
...
def compile_promote_offsets(node: nodes.PromoteOffsetsNode, ordered: bool = True):
    return compile_node(node.child, ordered).promote_offsets(node.col_id)
...
def compile_concat(node: nodes.ConcatNode, ordered: bool = True):
    if ordered:
        ...
        compiled_unordered = [compile_node(node, False) for node in node.children]
...
def compile_window(node: nodes.WindowOpNode, ordered: bool = True):
    return compile_node(node.child, ordered).project_window_op(
        node.column_name,
        node.op,
        node.window_spec,
        node.output_name,
