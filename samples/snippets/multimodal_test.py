...
def test_multimodal_dataframe(gcs_dst_folder: str) -> None:
    ...
    df_image["blurred"] = df_image["image"].blob.image_blur(
        (20, 20), dst=f"{dst_folder}/image_blur_transformed/"
    )
    df_image["resized"] = df_image["image"].blob.image_resize(
        (300, 200), dst=f"{dst_folder}/image_resize_transformed/"
    )
    df_image["normalized"] = df_image["image"].blob.image_normalize(
        alpha=50.0,
        beta=150.0,
        norm_type="minmax",
        dst=f"{dst_folder}/image_normalize_transformed/",
    )

    # You can also chain functions together
    df_image["blur_resized"] = df_image["blurred"].blob.image_resize(
        (300, 200), dst=f"{dst_folder}/image_blur_resize_transformed/"
    )
    df_image
